...
public class AnimationReader {
   ...
   * <ul>
   * <li>cs3500.model.Shape lines: the keyword "shape" followed by two identifiers (i.e.
   ...
   * @return ModelView returns a modelView model with the desired components
   */
   ...
   *
   * @param s       the scanner
   * @param builder the builder passed in
   * @param <Doc>   the type of builder
   */
  ...
  private static <Doc> void readShape(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException(
          "cs3500.model.Shape: Expected a name, but no more input available");
    }
    if (s.hasNext()) {
      type = s.next();
    } else {
      throw new IllegalStateException(
          "cs3500.model.Shape: Expected a type, but no more input available");
    }
  ...
  /**
   * Reads the rotation of a given shape and when.
   *
   * @param s       the scanner
   * @param builder the builder passed in
   * @param <Doc>   the type of builder
  ...
  private static <Doc> void readRotate(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException(
          "Rotation: Expected a shape name, but no more input available");
    }
    for (int i = 0; i < 2; i++) {
      vals[i] = getInt(s, "Rotate", fieldNames[i]);
    }

    builder.addRotation(name, vals[0], vals[1]);
  }
